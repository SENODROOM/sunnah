"""
sunan_al_nasai.nasai — Python library for Sunan al-Nasa'i.
...
Data source priority:
  1. data_path kwarg
  2. python/sunan_al_nasai/data/nasai.json.gz  (inside installed wheel)
  3. data/nasai.json.gz  (repo root — dev mode)
  4. data/nasai.json     (uncompressed fallback)
...
_INSTALLED_DATA = _THIS_DIR / "data" / "nasai.json.gz"
_SHARED_JSON    = _REPO_ROOT / "data" / "nasai.json.gz"
_SHARED_JSON_FB = _REPO_ROOT / "data" / "nasai.json"
...
class Nasai:
    """Complete Sunan al-Nasa'i collection. API identical to the npm package."""
    ...
    def __repr__(self) -> str: return f"<Nasai hadiths={len(self)} chapters={len(self.chapters)}>"
...
def _load_from_cdn() -> _Store:
    try: import requests
        ...
        raise RuntimeError("data/nasai.json.gz not found and 'requests' is not installed.\nEither bundle the data file or run: pip install requests")
    CDN_BASE = "https://cdn.jsdelivr.net/npm/sunan-al-nasai@1.0.0/data/chapters"

// CommonJS entry — require('sunan-al-nasai')
'use strict';
const fs   = require('fs');
...
class Nasai {
  constructor(nasaiData) {
    this._hadiths = nasaiData.hadiths;
    return new Proxy(this._hadiths, {
      get: (target, prop) => {
        if (!isNaN(prop))   return target[parseInt(prop)];
        if (prop in target) return target[prop];
        switch (prop) {
          case 'metadata':     return nasaiData.metadata;
          case 'chapters':     return nasaiData.chapters;
...
module.exports = nasai;
module.exports.Nasai   = Nasai;
module.exports.default = nasai;

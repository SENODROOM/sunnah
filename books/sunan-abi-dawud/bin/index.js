#!/usr/bin/env node
// CLI for sunan-abi-dawud
// Usage:
//   dawud <hadithId> [-a|-b]
//   dawud <chapterId> <hadithId> [-a|-b]
//   dawud --search "prayer"
//   dawud --search "prayer" --all
//   dawud --random
//   dawud --chapter <id>
//   dawud --react
//   dawud -h | --help
...
const dawudData = JSON.parse(
  fs.readFileSync(path.join(__dirname, "dawud.json"), "utf8"),
);
...
if (isRunDirectly()) {
  ...
  if (wantsVersion) {
    console.log("");
    console.log(
      "  " + bold(cyan("sunan-abi-dawud")) + gray(" v" + pkg.version),
    );
    console.log(
      "  " +
        gray("Total hadiths : ") +
        yellow(dawudData.hadiths.length.toLocaleString()),
    );
    console.log(
      "  " +
        gray("Total chapters: ") +
        yellow(dawudData.chapters.length.toLocaleString()),
    );
    console.log(
      "  " +
        gray("Author        : ") +
        cyan("Imam Abu Dawud Sulayman ibn al-Ash'ath"),
    );
    console.log("");
    process.exit(0);
  }
  ...
  if (wantsRandom) {
    const hadith =
      dawudData.hadiths[Math.floor(Math.random() * dawudData.hadiths.length)];
    printHadith(hadith);
    process.exit(0);
  }
  ...
  if (searchQuery !== null) {
    ...
    const results = dawudData.hadiths.filter(
      (h) =>
    ...
    if (!wantsAll && results.length > 5) {
      ...
      console.log("  " + dim('dawud --search "' + searchQuery + '" --all'));
      console.log("");
  ...
  if (wantsReact) {
    ...
    const CDN =
      "https://cdn.jsdelivr.net/npm/sunan-abi-dawud@" +
      pkg.version +
      "/chapters";

    const hookSrc = `// Auto-generated by: dawud --react
// Do not edit — re-run \`dawud --react\` to regenerate.
//
// Usage in any component:
//   import { useDawud } from '../hooks/useDawud';
//   const dawud = useDawud();
//   if (!dawud) return <p>Loading...</p>;
//   dawud.get(23)              // hadith by ID
//   dawud.search('prayer')     // search (all results)
//   dawud.search('prayer', 5)  // search (limit 5)
//   dawud.getRandom()          // random hadith
//   dawud.getByChapter(1)      // chapter hadiths
//   dawud[22]                  // index access
...
export function useDawud() {
  const [dawud, setDawud] = useState(_cache);
...
`;
    ...
    console.log("");
    console.log("    " + cyan("import { useDawud } from '../hooks/useDawud';"));
    console.log("");
    console.log("    " + gray("function MyComponent() {"));
    console.log("      " + gray("const dawud = useDawud();"));
    console.log("      " + gray("if (!dawud) return <p>Loading...</p>;"));
    console.log("      " + gray("return <p>{dawud.get(1).english.text}</p>;"));
    console.log("    " + gray("}"));
    console.log("");
    process.exit(0);
  }
  ...
  if (
    ...
    console.log("");
    console.log(
      "  " + bold(cyan("Sunan Abi Dawud CLI")) + gray("  v" + pkg.version),
    );
    console.log("");
    ...
    console.log("    " + dim("dawud 1"));
    console.log("    " + dim("dawud 2345 -a"));
    console.log("    " + dim("dawud 2345 -b"));
    console.log("    " + dim("dawud 23 34"));
    console.log("    " + dim('dawud --search "prayer"'));
    console.log("    " + dim('dawud --search "fasting" --all'));
    console.log("    " + dim("dawud --chapter 5"));
    console.log("    " + dim("dawud --random"));
    console.log("    " + dim("dawud --random -b"));
    console.log("    " + dim("dawud --react"));
    console.log("");
    process.exit(0);
  }

"""
sunan_abi_dawud.dawud — Python library for Sahih Dawud.
...
Data source priority:
  1. data_path kwarg
  2. data/dawud.json.gz  (repo root — auto-detected)
  3. data/dawud.json     (uncompressed fallback)
...
class Dawud:
    """
    Complete Sahih Dawud collection. API identical to the npm package.

    dawud = Dawud()
    dawud = Dawud(data_path="/path/to/dawud.json")
    """
    ...
    def search(self, query: str, limit: int = 0) -> List[Hadith]:
        q = query.lower()
        r = [
            h for h in self._s.hadiths if q in h.text.lower() or q in h.narrator.lower()
        ]
    ...
    def __repr__(self) -> str:
        return f"<Dawud hadiths={len(self)} chapters={len(self.chapters)}>"
...
def _load_from_cdn() -> _Store:
    try:
        ...
        raise RuntimeError(
            "data/dawud.json.gz not found and 'requests' is not installed.\n"
            "Either bundle the data file or run: pip install requests"
        )
    CDN_BASE = "https://cdn.jsdelivr.net/npm/sahih-dawud@1.0.2/data/chapters"
